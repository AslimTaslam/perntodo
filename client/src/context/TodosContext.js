...
export const TodosContextProvider = (props) => {
  ...
  const searchTodo = (items, searchItem) => {
    try{
      ...
      return items.filter((item) => {
        return item.description.toLowerCase().indexOf(searchItem.toLowerCase()) > -1;
      });
    }catch(err){
      console.error(err.message);
    }
  ...
  return (
    <TodosContext.Provider value={{ todos, filterTodos, setTodos, filter, setFilter, searchItem, setSearchItem, addTodo, updateTodo, searchTodo}}>
      { props.children }
